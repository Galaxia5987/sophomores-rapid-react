...
/**
 * A command that runs a `startCommand` when initialized and runs an `endCommand`
 * when it is finished.
 * Similar to the WPILib's `Commands.startEnd` function, but requires two Commands instead of two lambdas.
 *
 * @param startCommand The command to be executed when this command is initialized.
